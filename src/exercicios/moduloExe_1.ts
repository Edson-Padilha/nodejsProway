...
function escreveLogMemoria() {
    ...
    const conteudo = `Memória total: {memoriaTotal} MB \r\n
                      Memória Livre: {memoriaLivre} MB \r\n
                      Utilização da memória: {usoMemoria}`
                      
try {
    fs.appendFileSync(path.resolve("log.txt"), conteudo);
} catch (error) {
    console.error(error);
}
}
